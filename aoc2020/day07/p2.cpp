...
// 176035
int GetCount(vector<Rule>& rules, Bag& b) {
    int res = 1;
    for(Rule& r: rules) {
        if(b.compare(r.mainBag)) {
            if(r.subBags.size() == 0) {
                return 1;
            }
            for(pair<shared_ptr<Bag>, int> p: r.subBags) {
                res += p.second * GetCount(rules, *p.first);
            }
        }
    }
    return res;
...
int main() {
    ...
    long long Counter = 1;
    for(shared_ptr<Bag> b: confirmedBags) {
        for(Rule& r: Rules) {
            if( r.mainBag.get() == b.get()) {
                for(pair<shared_ptr<Bag>, int> p: r.subBags) {
                    cout << "c: " << to_string(Counter) << endl;
                    Counter += p.second;
                }
                break;
            }
