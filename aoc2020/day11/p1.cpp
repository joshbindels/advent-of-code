...
int get_empty_seats_in_direction(const vector<vector<POS_STATUS>>& inp, int pos_y, int pos_x) {
...
#define STAY    0
    ...
    for(int y=0; y<inp.size(); y++) {
        ...
        for(int x=0; x<inp[0].size(); x++) {
            ...
            int emptyseats = 0;
            emptyseats += get_empty_seats_in_direction(inp, y+UP, x+LEFT);
            emptyseats += get_empty_seats_in_direction(inp, y+UP, x);
            emptyseats += get_empty_seats_in_direction(inp, y+UP, x+RIGHT);
            emptyseats += get_empty_seats_in_direction(inp, y, x+LEFT);
            emptyseats += get_empty_seats_in_direction(inp, y, x+RIGHT);
            emptyseats += get_empty_seats_in_direction(inp, y+DOWN, x+LEFT);
            emptyseats += get_empty_seats_in_direction(inp, y+DOWN, x);
            emptyseats += get_empty_seats_in_direction(inp, y+DOWN, x+RIGHT);
            ...
            else if(inp[y][x] == TAKEN && emptyseats < 5) {
